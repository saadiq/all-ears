...
4. **The pipeline tools** — `transcribe`, `cleanup`, `summarize`. Independent binaries that read files (and, for streaming, tail the live index) and write Markdown outputs. The daemon spawns a chain of them when a session ends — but only the chain that session's starter declared, or, for a session that declared none, the per-trigger default (`on_end_stages`, applied to browser sessions).
...
**Capture (session-scoped):** the daemon boots idle. When a session starts (`session.start` from the browser extension or the CLI), the daemon starts capture of the session's sources; each engine appends encoded, time-stamped chunks — and its VAD appends speech/silence spans — under `sessions/<id>/sources/<sid>/`. Pause and resume are marks over the recording, never capture control. When the session ends, capture stops and the actors are torn down; the daemon then runs the on-end pipeline the session asked for at `session.start` — `transcribe --session <id>`, then `cleanup` and `summarize` over its output. A session that asked for nothing gets the per-trigger default: the configured `[earsd.sessions] on_end_stages` for browser sessions, nothing for anyone else.

**Native-app meeting detection:** a `MeetingActivityMonitor` inside `earsd` polls each configured `app:*` source's process audio input (via `EarsCaptureKit`'s Core Audio process-object probe — no tap, no TCC grant) and debounces the samples into episodes, per `[earsd.detection]`'s `debounce_s`. A confirmed begin/end edge is published on the daemon's live feed as `meeting.activity` telemetry and also drives `SessionRegistry`'s app-idle policy: an app-detected session auto-ends once every `app:*` source it names has gone quiet continuously for `idle_grace_s`, the same way a browser session auto-ends on an idle ingest stream. `status`'s `meeting_activity` list carries the monitor's current snapshot, so a late-connecting client catches up without waiting for the next edge.
