...
4. **The pipeline tools** — `transcribe`, `cleanup`, `summarize`. Independent binaries that read files (and, for streaming, tail the live index) and write Markdown outputs. The daemon spawns `transcribe --session <id>` itself when a browser-triggered session ends.
