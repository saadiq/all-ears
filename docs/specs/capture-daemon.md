...
- Browser-triggered sessions run the `[earsd.sessions] on_end_stages` chain automatically when they end (default `transcribe` → `cleanup` → `summarize`): the daemon spawns `transcribe --session <id> --json`, feeds the transcript path to `cleanup … --json`, and the cleaned path to `summarize … --all-presets --json`. Manual sessions run the pipeline by hand.
