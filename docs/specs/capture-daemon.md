...
- **System / per-app audio:** Core Audio **process taps** (`CATap`, macOS 14.4+): build a `CATapDescription` → `AudioHardwareCreateProcessTap` → wrap in a private auto-start **tap-only aggregate device** (no sub-device, to avoid duplicate/echo audio) for a clean IO proc. The tap's format is read from `kAudioTapPropertyFormat`, never assumed. ScreenCaptureKit is rejected for this: it can't isolate per-app audio and forces a screen-recording prompt.
  - **Per-app scoping** (`app:<bundle-id>`) uses the tap's process-inclusion list: the daemon resolves a bundle id to its live PIDs via `NSWorkspace`, tracks process launch/exit, and rebuilds the inclusion list as the app's processes come and go. A bundle id `NSWorkspace` cannot see — a media daemon such as FaceTime's `com.apple.avconferenced`, which owns the call's audio while FaceTime.app only draws it — is resolved through the HAL's process-object list instead, **once, at backend build**: `NSWorkspace` never posts launch/exit for a daemon, so that pid set is not re-resolved if the daemon had not yet touched audio or relaunches mid-session. Inclusion/exclusion semantics are covered by integration tests; full isolation verification needs an opt-in test on real hardware with the permission granted.
...
- Recording is session-scoped and sessions are started deliberately — by the browser extension, the CLI, or an accepted menu-bar detection prompt. Meeting detection never starts capture on its own; it only asks.
...
- **A session that declares nothing falls back per trigger.** Browser-extension and app-detected sessions inherit `[earsd.sessions] on_end_stages` (default `transcribe` → `cleanup` → `summarize`) — neither starter has another way to ask, and both flows assume a transcript appears; every other trigger runs nothing. So `ears session start`/`session end` stays inert unless asked — a scripted capture never silently grows a model load and a per-preset LLM bill.
- When a chain does run, the daemon spawns `transcribe --session <id> --job-id <job> --json`, feeds the transcript path to `cleanup … --json`, and the cleaned path to `summarize … --select-preset --json` (one preset per session, chosen by classifying the transcript against each preset's `when`). `on_end_stages = []` in config disables the browser fallback too.
...
- **The chain stops after transcribe when the transcript is empty.** A session that recorded only background audio still transcribes, into a document with nothing in it, and the LLM stages downstream invent content from that and publish the invention: the 2026-08-20 `c08595c8` session ran 605 seconds, transcribed to the single word `Yeah.`, and its summary — speculating in prose about whether the meeting had happened — was written over an unrelated daily note. The gate reads the fresh transcript's own `word_count` and `speech_seconds`; below `[earsd.sessions] min_words` (default 10) *or* `min_speech_seconds` (default 5), `cleanup` and `summarize` are skipped and the daemon logs one line naming the measurements against the thresholds. Either threshold at `0` disables that half; both at `0` runs the chain unconditionally. A skipped chain is a *successful* chain — the transcript is still written and `transcript_completed` is still stamped, so audio retention is unaffected — and `ears session show` renders the skipped stages as `skipped (empty transcript)` rather than as missing artifacts.
...
- Each chain run records what its stages reported as the session's `pipeline_issues`: a failed stage (its error message and exit class) and each `warning:` line a successful stage wrote. A new run replaces the list. `ears sessions` names the failed stage and class (`✗ transcribed, summarize failed (retryable-upstream)`), `ears session show` prints the message in that stage's row, and stage warnings count toward the warnings total.
...
Every subcommand has concise `--help`. Output is human-readable by default — `status` renders a dashboard that groups sources under the session they feed, labels browser sources only with names the session's speaker map or roster provides and app sources with their configured meeting label (`Zoom`, never a name invented from the id), and ends with a short recent-sessions tail — with `--json` for scripting (`status` and `session list` keep their existing payload shapes; additions are additive-only). Exits non-zero with a clear message if the daemon is unreachable; `sessions --all` and `session show` read from disk and need no daemon at all. Both views read a session against the on-end chain it actually declared (or inherited, per its trigger): a stage the session never asked for renders as `not requested`, a stage it did ask for that the empty-transcript gate stopped renders as `skipped (empty transcript)`, and the last stage it did ask for completing is its terminal success — an `ears session start` capture that runs nothing is `recorded`, not a pipeline that failed.
