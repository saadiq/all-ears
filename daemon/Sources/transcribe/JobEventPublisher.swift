...
/// Best-effort publisher of `job.publish` progress for a session-level
...
actor JobEventPublisher {
  ...
  init(
    socketPath: String?, jobID: String, sessionID: String,
  ...
  func publish(state: JobState, detail: String? = nil) async {
    ...
    do {
      _ = try await client.send(
        .jobPublish(
          JobPublishParams(
            job: jobID, kind: "transcribe", session: sessionID, state: state, detail: detail)),
