...
/// `transcribe`'s batch `--json` result envelope (issue #63), schema
/// `allears.transcribe/v1` — the versioned, additive-friendly result surface
...
///   (the `.transcript.md`); `outputs` lists every written artifact (the
///   markdown plus its `.transcript.json` sidecar).
...
/// (`shared/stage-envelopes/transcribe.v1.schema.json`); this struct is the
/// tool-local `Codable` twin. Kept in the tool on purpose — the daemon gets
/// its own decoder in the consumer issue, the schema file is the shared
/// artifact.
struct TranscribeResultEnvelope: Codable, Sendable, Equatable {
  /// The frozen schema identifier every v1 envelope carries.
  static let schemaID = "allears.transcribe/v1"

  var schema: String
  var ok: Bool
  /// Success only: the primary artifact's absolute path (`.transcript.md`).
  ...
  var stats: Stats? = nil
  /// Failure only: the exit-code taxonomy label (issue #61), e.g.
  /// `"input-missing"`.
